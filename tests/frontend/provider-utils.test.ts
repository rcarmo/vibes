...
const providers = [
  ...
  { id: 'copilot', label: 'Copilot', available: true, status: 'ready', transport: 'acp', capabilities: { permission_requests: true, fs_read_text_file: true, fs_write_text_file: false, terminal_services: false }, session_metadata: { modes: ['default', 'plan'], config_options: [{ id: 'mode' }], commands: [{ id: 'explain' }], current_mode: 'default' } },
];

describe('provider utilities', () => {
  test('normalizes provider payloads', () => {
    expect(normalizeProviders({ providers })).toHaveLength(3);
  ...
  test('finds and filters providers', () => {
    ...
    expect(getAvailableProviders(providers).map((provider) => provider.id)).toEqual(['pi', 'copilot']);
  });
  ...
  test('summarizes capabilities and selection', () => {
    ...
    expect(providerCapabilitySummary(providers[2])).toEqual(['permissions', 'read-only fs', 'modes', 'config', 'commands']);
