...
async function capture(browser,app,scenario,view,pass){
 ...
 try{
  ...
  const geometry=await page.evaluate(()=>Object.fromEntries(['.compose-box','.timeline','.agent-status-panel','.compose-session-popup','.compose-model-popup','.timeline-quick-actions','.system-meters-hud','.post','#post-103 img','.compose-input-wrapper','.compose-input-main','.compose-box textarea','.compose-footer','.compose-footer-left','.compose-actions','.compose-context-group','.compose-model-hint-btn','.compose-session-switcher-btn','.compose-context-pie','.agent-thinking-title','.agent-thinking-body','.compose-model-catalogue-results','.compose-model-catalogue-footer','.compose-model-catalogue-footer-start','.compose-model-catalogue-thinking','.compose-model-catalogue-thinking select','.compose-model-catalogue-option','.compose-model-catalogue-option-content','.compose-model-catalogue-pin','.post-actions','.post-code-copy-btn','.post-code-copy-btn svg','.post-code-copy-btn svg path','.post-code-copy-btn svg rect','.compose-session-trigger span','.post-content code span','.compose-session-label','.compose-session-pill-caret','.post-content pre','.post-content code','.compose-input-main','.compose-session-trigger-label','.compose-session-trigger-icon','.post-copy-btn','.post-copy-btn svg','.post-copy-btn svg path','.post-copy-btn svg rect','.post-delete-btn svg path','.compose-send-stack svg path','.post-delete-btn','.compose-send-stack','.compose-send-stack button','.compose-session-trigger','.compose-send-stack svg','.compose-session-switcher-btn svg','.compose-session-trigger','.compose-session-trigger-caret','.compose-model-usage-hint','.agent-status-text','.agent-status','.agent-status .turn-dot','.compose-queue-stack-item','.compose-queue-stack-text','.compose-queue-stack-actions','.compose-session-search','.compose-session-popup-header','.compose-session-popup-results','.compose-session-row-content','.compose-session-row-main','.compose-session-row-meta','.session-item','.compose-model-popup-actions','.compose-session-section-heading','.post-file-refs','.post-file-pill','.post-file-name','#post-103 .post-content','#post-103 .post-meta','#post-103 .media-preview'].map(selector=>[selector,[...document.querySelectorAll(selector)].map(el=>{const b=el.getBoundingClientRect(),s=getComputedStyle(el);return {x:b.x,y:b.y,width:b.width,height:b.height,font:s.font,fontSize:s.fontSize,color:s.color,background:s.backgroundColor,padding:s.padding,border:s.border,display:s.display,appearance:s.appearance,justifyContent:s.justifyContent,alignItems:s.alignItems,gap:s.gap,lineHeight:s.lineHeight,boxSizing:s.boxSizing,position:s.position,margin:s.margin,opacity:s.opacity,fontWeight:s.fontWeight,letterSpacing:s.letterSpacing,verticalAlign:s.verticalAlign,transform:s.transform,html:['BUTTON','SELECT','CODE'].includes(el.tagName) ? el.outerHTML : undefined,fill:s.fill,stroke:s.stroke,strokeWidth:s.strokeWidth,strokeLinecap:s.strokeLinecap,svg:el.querySelector('svg')?.outerHTML || (el.tagName.toLowerCase()==='svg' ? el.outerHTML : undefined)};})])));
