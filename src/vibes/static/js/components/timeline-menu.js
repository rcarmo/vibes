...
export function TimelineMenu({ workspaceOpen, onToggleWorkspace, onOpenTerminal, onOpenQuickActions, onToggleHidden }) {
    ...
    return html`<div class=${`timeline-menu-portal ${workspaceOpen ? 'in-workspace' : 'in-chat'}`} style="top:max(8px, env(safe-area-inset-top));left:8px;right:auto">
        <button ref=${button} class=${`timeline-menu-btn${open ? ' active' : ''}`} data-testid="hamburger"
            type="button" title="Menu" aria-label="Workspace menu" aria-haspopup="menu"
        ...
        ${open && html`<div ref=${menu} class="workspace-menu-dropdown timeline-menu-dropdown" role="menu">
            ...
            ${workspaceOpen && onToggleHidden && html`<button class="workspace-menu-item" role="menuitem" onClick=${() => run(onToggleHidden)}>Toggle hidden files</button>`}
