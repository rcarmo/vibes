    ...
    return html`<div class="compose-model-popup compose-session-popup" data-testid="session-popup" aria-busy=${busy} onKeyDown=${keys}>
        ...
        <div ref=${results} id="session-picker-results" class="compose-model-popup-menu compose-session-popup-results" role="listbox" aria-label="Sessions" aria-activedescendant=${matches[selectedIndex] ? `session-option-${matches[selectedIndex].id}` : undefined}>
            ${groups.map(group => html`<div class="session-popup-group" role="group" aria-label=${group.label}>
                ...
                <button type="button" id=${`session-option-${item.id}`} class=${`compose-model-popup-item session-item${item.archived ? ' archived' : item.id === currentId ? ' current' : ''}`} role="option" aria-selected=${item.id === currentId} onClick=${() => act(() => onSelect?.(item.id))}>
                    ...
                    <span class=${`compose-session-status-pill ${item.archived ? 'archived' : item.is_running ? 'active' : 'idle'}`}>${item.archived ? 'Archived' : item.is_running ? 'Running' : 'Idle'}</span>
        ...
        <div class="compose-model-popup-actions">
            ...
            <button type="button" class="compose-model-popup-btn" title="Create an independent root session" onClick=${() => act(onCreate)}>New root…</button>
