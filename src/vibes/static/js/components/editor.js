...
export function WorkspaceEditor({
    ...
    const [vimEnabled, setVimEnabled] = useState(() => {
        try {
            return localStorage.getItem('vibes_vim_mode') === 'true';
        } catch {
            return false;
        }
    });
    ...
    const [showWhitespace, setShowWhitespace] = useState(() => {
        try {
            const stored = localStorage.getItem('vibes_show_whitespace');
    ...
    useEffect(() => {
        ...
        try {
            localStorage.setItem('vibes_vim_mode', vimEnabled ? 'true' : 'false');
        } catch {
            // ignore
        }
    ...
    useEffect(() => {
        try {
            localStorage.setItem('vibes_show_whitespace', showWhitespace ? 'true' : 'false');
        } catch {
            // ignore
        }
        const view = viewRef.current;
